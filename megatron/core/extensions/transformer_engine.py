...
def _get_fp8_autocast_for_quant_recipe(
    ...
    if qrecipe.fp8_quantization_recipe is None and qrecipe.fp4_quantization_recipe is None:
        ...
        if (
            ...
            quant_recipe = _get_custom_recipe(qrecipe.custom_recipe_factory, config=config)
...
def _get_fp8_autocast_for_quant_params(
    ...
    if qparams is None:
        ...
        return _get_fp8_autocast_for_quant_recipe(qparams.evaluation_recipe, config=config)
    else:
        return _get_fp8_autocast_for_quant_recipe(qparams.training_recipe, config=config)
...
def _set_custom_fp4_tp_amax_reduction(
    quantizer: Any,
    *,
    enabled: bool,
    group: Optional[torch.distributed.ProcessGroup],
) -> None:
    if quantizer is None or not getattr(quantizer, "custom", False):
        return
    setattr(quantizer, "with_amax_reduction", enabled)
    setattr(quantizer, "amax_reduction_group", group if enabled else None)
    sync = getattr(quantizer, "_sync_internal_quantizers", None)
    if callable(sync):
        sync()


def _customize_custom_fp4_quantizers_for_module(
    module: Any,
    quantizers: Any,
    *,
    is_grad_enabled: Optional[bool] = None,
):
    if getattr(getattr(module, "config", None), "fp4_recipe", None) != Fp4Recipe.custom:
        return quantizers
    quantizers = list(quantizers)
    if len(quantizers) < 6:
        return tuple(quantizers)

    input_quantizer = quantizers[0]
    weight_quantizer = quantizers[1]
    output_quantizer = quantizers[2]
    grad_output_quantizer = quantizers[5]

    # Preserve the outer module-forward grad mode across TE's custom autograd
    # boundary. Fused LayerNormLinear creates its normalized activation inside
    # that boundary, where the tensor itself may not require grad in training.
    if input_quantizer is not None:
        setattr(
            input_quantizer,
            "_fp4_forward_grad_enabled",
            None if is_grad_enabled is None else bool(is_grad_enabled),
        )

    try:
        from megatron.core.extensions.fp4_outlier_recipe import set_layer_name

        module_name = getattr(module, "_megatron_module_path", None)
        if not module_name:
            module_name = f"{module.__class__.__name__}:{id(module)}"
        set_layer_name(input_quantizer, f"{module_name}.input_quantizer")
        set_layer_name(weight_quantizer, f"{module_name}.weight_quantizer")
        set_layer_name(grad_output_quantizer, f"{module_name}.grad_output_quantizer")
    ...
    if sequence_parallel and parallel_mode == "column":
        _set_custom_fp4_tp_amax_reduction(input_quantizer, enabled=True, group=tp_group)
    if sequence_parallel and parallel_mode == "row":
        _set_custom_fp4_tp_amax_reduction(grad_output_quantizer, enabled=True, group=tp_group)
    return tuple(quantizers)
...
class TELinear(te.pytorch.Linear):
    ...
    def _get_quantizers(self, fp8_output, fp8_grad, is_grad_enabled):
        quantizers = super()._get_quantizers(fp8_output, fp8_grad, is_grad_enabled)
        return _customize_custom_fp4_quantizers_for_module(
            self,
            quantizers,

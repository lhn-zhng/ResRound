...
_LOGGED_RECIPE_PATHS: set[str] = set()
...
class RecipeConfig:
    ...
    def from_yaml_file(recipe_yaml_path: str) -> "RecipeConfig":
        ...
        recipe_log_key = os.path.abspath(recipe_yaml_path)
        if recipe_log_key not in _LOGGED_RECIPE_PATHS:
            _LOGGED_RECIPE_PATHS.add(recipe_log_key)
            log_single_rank(
                logger,
                logging.INFO,

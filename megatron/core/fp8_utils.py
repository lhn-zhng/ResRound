...
def _resolve_module_and_callable_from_python_import_path(dotted_path: str):
...
def _resolve_callable_from_python_import_path(dotted_path: str):
    _mod, fn = _resolve_module_and_callable_from_python_import_path(dotted_path)
    return fn
...
def _get_custom_recipe(
    ...
    mod, quantizer_factory = _resolve_module_and_callable_from_python_import_path(
        quantizer_factory_python_path
    )
    configure = getattr(mod, "configure_from_transformer_config", None)

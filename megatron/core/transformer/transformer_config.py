...
class TransformerConfig(ModelParallelConfig):
    ...
    fp4_outlier_ratio: float = 0.01
    """Top absolute-value ratio reserved as exact sparse input outliers for FP4 FPROP."""

    fp4_outlier_selection_method: Literal['topk', 'normal_threshold'] = "topk"
    """How FP4 FPROP input outliers are selected.
    'topk' performs exact absolute-value top-k selection. 'normal_threshold' selects values
    beyond a Gaussian-tail threshold estimated from the current tensor."""

    fp4_outlier_adaptive_ratio: bool = False
    """Dynamically adjust the input outlier ratio from each tensor's tail heaviness."""

    fp4_outlier_adaptive_min_ratio: float = 0.0
    """Minimum adaptive outlier ratio after the 0.001% zero cutoff."""

    fp4_outlier_adaptive_max_ratio: float = 0.01
    """Maximum adaptive outlier ratio."""

    fp4_outlier_adaptive_reference_heaviness: float = 15.0
    """Reference max/mean_abs heaviness where fp4_outlier_ratio is used unchanged."""

    fp4_outlier_enable_fprop: bool = True
    """Whether FPROP uses the custom sparse input-outlier path."""

    fp4_outlier_enable_fast_fprop: bool = False
    """Use the optional r207 select+quant and direct sparse-correction kernels for FPROP."""

    fp4_outlier_enable_dgrad: bool = False
    """Reserved for compatibility. This clean branch supports only FPROP input outliers."""

    fp4_outlier_enable_wgrad: bool = False
    """Reserved for compatibility. This clean branch supports only FPROP input outliers."""

    fp4_outlier_enable_nvfp4_a1_a2_all_gather: bool = False
    """When sequence parallel all-gather is used for FPROP inputs, communicate local
    NVFP4(A1) plus the sparse A2 payload instead of gathering the high-precision input and
    re-quantizing after gather."""

    fp4_outlier_enable_weight_rounding: bool = False
    """Ephemerally override a sparse set of FPROP weight E2M1 codes using the
    current quantized dense-main activation's local covariance. Master weights,
    NVFP4 scales, and the DGRAD/WGRAD weight variants remain unchanged."""

    fp4_outlier_weight_rounding_group_size: int = 64
    """Input-channel covariance block size for ephemeral FPROP weight rounding."""

    fp4_outlier_weight_rounding_rounds_per_group: int = 1
    """Greedy adjacent-code updates per output row and covariance group."""

    fp4_outlier_weight_rounding_selection_tokens: int = 2048
    """Maximum strided first-microbatch tokens used to estimate each local covariance."""

    fp4_outlier_weight_rounding_stratified_sampling: bool = False
    """Sample calibration tokens across both sequence positions and batch lanes,
    avoiding flattened-stride aliasing with the microbatch dimension."""

    fp4_outlier_weight_rounding_stratified_batch_size: int = 0
    """Local microbatch size used to restore the sequence/batch layout after TE
    flattens a linear input to [tokens, hidden]. Required by stratified sampling."""

    fp4_outlier_weight_rounding_crossfit_audit: bool = False
    """Split the sampled tokens into disjoint proposal and audit sets. Candidate
    code moves are selected only on the proposal set and rejected row-wise when
    they fail the held-out audit."""

    fp4_outlier_weight_rounding_combined_audit: bool = False
    """Audit candidate weight-code moves against the complete dense-main FPROP
    residual, including both input and weight quantization error. This avoids
    accepting a weight-only improvement that destroys beneficial error
    cancellation with the quantized input."""

    fp4_outlier_weight_rounding_joint_objective: bool = False
    """Include the input-quantization cross term when proposing FPROP weight
    code moves, rather than proposing on weight-only reconstruction and using
    the complete residual only as a rejection audit."""

    fp4_outlier_weight_rounding_audit_tokens: int = 512
    """Number of sampled tokens reserved for the held-out cross-fit audit."""

    fp4_outlier_weight_rounding_audit_max_regression_fraction: float = 0.5
    """Maximum fraction of held-out tokens whose squared scalar output error may
    measurably increase for an accepted output-row update."""

    fp4_outlier_weight_rounding_audit_min_relative_gain: float = 0.0
    """Minimum relative held-out output-error energy reduction required for an
    accepted output-row update. Zero preserves the original audit behavior."""

    fp4_outlier_weight_rounding_offdiag_shrink: float = 1.0
    """Shrink local covariance off-diagonal entries toward zero. A value of 1
    uses the empirical covariance unchanged; 0 keeps only its diagonal."""

    fp4_outlier_weight_rounding_expansion_only: bool = True
    """Apply ephemeral weight rounding only when output width is at least input width.
    This covers QKV, O projection, and FC1 while skipping the empirically weak FC2."""

    fp4_outlier_weight_rounding_min_expansion_ratio: float = 1.0
    """Minimum output/input width ratio when expansion-only rounding is enabled.
    Values just above one exclude square projections while retaining QKV and FC1."""

    fp4_outlier_weight_rounding_max_expansion_ratio: float = 0.0
    """Optional maximum output/input width ratio for expansion-only rounding.
    Zero disables the upper bound; a value of four keeps LLaMA QKV while
    excluding its wider FC1."""

    fp4_outlier_weight_rounding_layer_start: int = 0
    """First zero-based transformer layer allowed to use ephemeral weight rounding."""

    fp4_outlier_weight_rounding_layer_end: int = -1
    """Exclusive transformer-layer endpoint for ephemeral weight rounding.
    Negative values keep all remaining layers enabled."""

    fp4_outlier_weight_rounding_qkv_layer_end: int = -1
    """Optional QKV-specific exclusive layer endpoint. Negative values inherit
    the global layer endpoint; zero disables QKV weight rounding."""

    fp4_outlier_weight_rounding_proj_layer_end: int = -1
    """Optional attention-output-projection exclusive layer endpoint. Negative
    values inherit the global layer endpoint; zero disables this module."""

    fp4_outlier_weight_rounding_fc1_layer_end: int = -1
    """Optional FC1-specific exclusive layer endpoint. Negative values inherit
    the global layer endpoint; zero disables FC1 weight rounding."""

    fp4_outlier_weight_rounding_reuse_generation_payload: bool = True
    """Build the FPROP-only rounded payload from the first microbatch after each
    weight update, then reuse it until the quantized weight generation changes."""

    fp4_outlier_weight_rounding_dgrad_consistency: bool = False
    """Mirror each selected FPROP weight-code move into a columnwise NVFP4 view
    used by DGRAD. The master weight and WGRAD path remain unchanged."""

    fp4_outlier_store_input_dense_main: bool = False
    """Keep the dense A1 tensor on the custom input qresult for debugging."""

    fp4_outlier_main_quantizer_rht: bool = False
    """Enable RHT on the custom dense-main NVFP4 quantizer."""

    fp4_outlier_input_stochastic_rounding: bool = False
    """Enable stochastic rounding for the custom FPROP input dense-main quantizer."""
    ...
    def __post_init__(self):
        ...
        if self.fp4 and self.fp4_recipe == Fp4Recipe.custom:
            ...
            if not 0.0 <= self.fp4_outlier_ratio <= 1.0:
                raise ValueError(
                    f"fp4_outlier_ratio must be in [0, 1], got {self.fp4_outlier_ratio}."
                )
            if self.fp4_outlier_selection_method not in {"topk", "normal_threshold"}:
                raise ValueError(
                    "fp4_outlier_selection_method must be 'topk' or 'normal_threshold'. "
                    f"Got '{self.fp4_outlier_selection_method}'."
                )
            if not 0.0 <= self.fp4_outlier_adaptive_min_ratio <= 1.0:
                raise ValueError(
                    "fp4_outlier_adaptive_min_ratio must be in [0, 1], "
                    f"got {self.fp4_outlier_adaptive_min_ratio}."
                )
            if not 0.0 <= self.fp4_outlier_adaptive_max_ratio <= 1.0:
                raise ValueError(
                    "fp4_outlier_adaptive_max_ratio must be in [0, 1], "
                    f"got {self.fp4_outlier_adaptive_max_ratio}."
                )
            if self.fp4_outlier_adaptive_min_ratio > self.fp4_outlier_adaptive_max_ratio:
                raise ValueError(
                    "fp4_outlier_adaptive_min_ratio must be <= "
                    "fp4_outlier_adaptive_max_ratio."
                )
            if self.fp4_outlier_adaptive_reference_heaviness <= 0.0:
                raise ValueError(
                    "fp4_outlier_adaptive_reference_heaviness must be positive, "
                    f"got {self.fp4_outlier_adaptive_reference_heaviness}."
                )
            if self.fp4_outlier_enable_fast_fprop and not self.fp4_outlier_enable_fprop:
                raise ValueError(
                    "fp4_outlier_enable_fast_fprop requires fp4_outlier_enable_fprop=True."
                )
            if self.fp4_outlier_enable_weight_rounding:
                if not self.fp4_outlier_enable_fprop:
                    raise ValueError(
                        "fp4_outlier_enable_weight_rounding requires "
                        "fp4_outlier_enable_fprop=True."
                    )
                if self.fp4_outlier_weight_rounding_group_size <= 0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_group_size must be positive."
                    )
                if self.fp4_outlier_weight_rounding_group_size % 16 != 0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_group_size must be a multiple "
                        "of the NVFP4 microscale width 16."
                    )
                if self.fp4_outlier_weight_rounding_rounds_per_group <= 0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_rounds_per_group must be positive."
                    )
                if (
                    self.fp4_outlier_weight_rounding_rounds_per_group
                    > self.fp4_outlier_weight_rounding_group_size
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_rounds_per_group cannot exceed "
                        "fp4_outlier_weight_rounding_group_size."
                    )
                if self.fp4_outlier_weight_rounding_selection_tokens <= 0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_selection_tokens must be positive."
                    )
                if (
                    self.fp4_outlier_weight_rounding_stratified_sampling
                    and self.fp4_outlier_weight_rounding_stratified_batch_size <= 0
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_stratified_batch_size must be "
                        "positive when stratified sampling is enabled."
                    )
                if self.fp4_outlier_weight_rounding_crossfit_audit:
                    if not (
                        0
                        < self.fp4_outlier_weight_rounding_audit_tokens
                        < self.fp4_outlier_weight_rounding_selection_tokens
                    ):
                        raise ValueError(
                            "fp4_outlier_weight_rounding_audit_tokens must be between "
                            "zero and fp4_outlier_weight_rounding_selection_tokens when "
                            "cross-fit audit is enabled."
                        )
                    if not (
                        0.0
                        <= self.fp4_outlier_weight_rounding_audit_max_regression_fraction
                        <= 1.0
                    ):
                        raise ValueError(
                    ...
                    if not (
                        0.0
                        <= self.fp4_outlier_weight_rounding_audit_min_relative_gain
                        <= 1.0
                    ):
                        raise ValueError(
                ...
                if (
                    self.fp4_outlier_weight_rounding_combined_audit
                    and not self.fp4_outlier_weight_rounding_crossfit_audit
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_combined_audit requires "
                        "fp4_outlier_weight_rounding_crossfit_audit=True."
                    )
                if (
                    self.fp4_outlier_weight_rounding_joint_objective
                    and not self.fp4_outlier_weight_rounding_combined_audit
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_joint_objective requires "
                        "fp4_outlier_weight_rounding_combined_audit=True."
                    )
                if not (
                    0.0 <= self.fp4_outlier_weight_rounding_offdiag_shrink <= 1.0
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_offdiag_shrink must be in [0, 1]."
                    )
                if self.fp4_outlier_weight_rounding_min_expansion_ratio < 1.0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_min_expansion_ratio must be >= 1."
                    )
                if (
                    self.fp4_outlier_weight_rounding_max_expansion_ratio != 0.0
                    and self.fp4_outlier_weight_rounding_max_expansion_ratio
                    < self.fp4_outlier_weight_rounding_min_expansion_ratio
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_max_expansion_ratio must be "
                        "zero or >= fp4_outlier_weight_rounding_min_expansion_ratio."
                    )
                if self.fp4_outlier_weight_rounding_layer_start < 0:
                    raise ValueError(
                        "fp4_outlier_weight_rounding_layer_start must be non-negative."
                    )
                if (
                    self.fp4_outlier_weight_rounding_layer_end >= 0
                    and self.fp4_outlier_weight_rounding_layer_end
                    <= self.fp4_outlier_weight_rounding_layer_start
                ):
                    raise ValueError(
                        "fp4_outlier_weight_rounding_layer_end must be negative "
                        "or greater than fp4_outlier_weight_rounding_layer_start."
                    )
                module_layer_ends = {
                    "qkv": self.fp4_outlier_weight_rounding_qkv_layer_end,
                    "proj": self.fp4_outlier_weight_rounding_proj_layer_end,
                    "fc1": self.fp4_outlier_weight_rounding_fc1_layer_end,
                }
                for module_name, module_layer_end in module_layer_ends.items():
                    if module_layer_end < -1:
                        raise ValueError(
                            "fp4_outlier_weight_rounding_"
                            f"{module_name}_layer_end must be >= -1."
                        )
                if self.fp4_outlier_main_quantizer_rht:
                    raise ValueError(
                        "fp4_outlier_enable_weight_rounding currently requires "
                        "fp4_outlier_main_quantizer_rht=False."
                    )
            if (
                self.fp4_outlier_weight_rounding_dgrad_consistency
                and not self.fp4_outlier_enable_weight_rounding
            ):
                raise ValueError(
                    "fp4_outlier_weight_rounding_dgrad_consistency requires "
                    "fp4_outlier_enable_weight_rounding=True."
                )
            if self.fp4_outlier_enable_dgrad or self.fp4_outlier_enable_wgrad:
                raise ValueError(
                    "This branch only supports FP4 FPROP input outliers. "
                    "Keep fp4_outlier_enable_dgrad=False and fp4_outlier_enable_wgrad=False."
                )

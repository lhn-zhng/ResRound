ResRound Research Release
=========================

This repository contains the reference implementation for **ResRound:
Improving Native NVFP4 Training via Sparse Activation Residuals and
Activation-Aware Weight Rounding** by LuHan Zhang, Xinrui Yang, and Shaohuai
Shi (corresponding author).

ResRound adds two forward-pass mechanisms to Megatron-LM: sparse activation
residuals (SAR) and activation-aware weight rounding (AWR). The release also
includes the CUDA sources used by the fused select/quantize and dense/sparse
execution paths. See [RESROUND_RELEASE.md](RESROUND_RELEASE.md) for the code
map, tested environment, dependencies, and reproduction entry points.

The remainder of this README is the upstream Megatron-LM documentation. The
original Apache-2.0 license is preserved in [LICENSE](LICENSE).
